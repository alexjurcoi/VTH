...
def make_output_filename(
    ...
    freq_array=None,
    beta=None,
    dG_lo=None, dG_hi=None, dG_step=None,
    duty=None,
    voltage=None,
    mech_label=None,
    base="dynamic_simulation_bruteforce_output.xlsx"
):
    """
    Build a unique filename string with parameters included.
    Includes sweep (dG_lo, dG_hi, dG_step) and duty cycle.
    """
    ...
    if freq_array is None:
        ...
        freq_str = "-".join([f"{float(f):.1e}" for f in freq_array])
    ...
    if beta is None:
        ...
        beta_str = "-".join([f"{float(b):.3f}" for b in beta])

    # ---- sweep string ----
    if (dG_lo is None) or (dG_hi is None) or (dG_step is None):
        sweep_str = "dGsweep=NA"
    else:
        sweep_str = f"dGsweep={dG_lo:.2f}_to_{dG_hi:.2f}_step_{dG_step:.3f}"

    # ---- duty string ----
    duty_str = "duty=NA" if duty is None else f"duty={float(duty):.3f}"

    # ---- voltage string ----
    volt_str = "V=NA" if voltage is None else f"V={float(voltage):.3f}"

    # ---- mechanism string (optional) ----
    mech_str = "" if mech_label is None else f"_{mech_label}"

    # Use base name (without extension) then add .xlsx
    ...
    filename = (
        f"{stem}{mech_str}_"
        f"{k_str}_freq={freq_str}_beta={beta_str}_"
        f"{sweep_str}_{duty_str}_{volt_str}"
        f"{ext}"
    )
    ...
    while os.path.exists(final_filename):
        final_filename = f"{stem}{mech_str}_{k_str}_freq={freq_str}_beta={beta_str}_{sweep_str}_{duty_str}_{volt_str}_{counter}{ext}"
        counter += 1
...
freq_norm_array = np.array([1e-2, 1e-1, 1e0, 1e1, 1e2])
...
def plot_block_heatmap(Z, dG_vals, title="", cbar_label="", norm=None):
    ...
    m = ax.pcolormesh(X, Y, Z, shading="flat", norm=norm)  # crisp blocks
...
if do_dynamic_ghad:
    ...
    # -------- CURRENT heatmap norms --------
    norm_curr = {}
    norm_rate = {}
    
    # ---- CURRENT norms (only if mech exists / has data) ----
    if len(heatmaps_curr["VT"]) > 0:
        vals = []
        for Z in heatmaps_curr["VT"].values():
            vals.append(Z[np.isfinite(Z)])
        vals = np.concatenate(vals)
        norm_curr["VT"] = Normalize(vmin=vals.min(), vmax=vals.max())
    
    if len(heatmaps_curr["VH"]) > 0:
        vals = []
        for Z in heatmaps_curr["VH"].values():
            vals.append(Z[np.isfinite(Z)])
        vals = np.concatenate(vals)
        norm_curr["VH"] = Normalize(vmin=vals.min(), vmax=vals.max())
    
    # ---- RATE norms (only if mech exists / has data) ----
    if len(heatmaps_rate["VT"]) > 0:
        vals = []
        for Z in heatmaps_rate["VT"].values():
            vals.append(Z[np.isfinite(Z)])
        vals = np.concatenate(vals)
        norm_rate["VT"] = Normalize(vmin=vals.min(), vmax=vals.max())
    
    if len(heatmaps_rate["VH"]) > 0:
        vals = []
        for Z in heatmaps_rate["VH"].values():
            vals.append(Z[np.isfinite(Z)])
        vals = np.concatenate(vals)
        norm_rate["VH"] = Normalize(vmin=vals.min(), vmax=vals.max())
    ...
    for mechanism_choice in mechanisms_to_run:
        ...
        for freq in freq_array:
            ...
            plot_block_heatmap(
                ...
                norm=norm_curr.get(mech_label, None)
            )
    ...
    for mechanism_choice in mechanisms_to_run:
        mech_label = MECH_LABEL[mechanism_choice]
        rate_label = heatmap_metric_VT if mech_label == "VT" else heatmap_metric_VH
        for freq in freq_array:
            Z = heatmaps_rate[mech_label][float(freq)]
            plot_block_heatmap(
                Z, dG_vals,
                title=f"{mech_label} {rate_label},\n Duty={duty} Voltage = {V_app} @ {freq:.2e} Hz",
                cbar_label=rate_label,
                norm=norm_rate.get(mech_label, None)
            )
   ...
   # ======================== EXCEL EXPORT (ALL TOGETHER) ========================
    ...
    filename = make_output_filename(
        ...
        freq_array=freq_norm_array,
        beta=beta,
        dG_lo=dG_lo, dG_hi=dG_hi, dG_step=dG_step,
        duty=duty,
        voltage=V_app,
        mech_label="VT"  # or "VH" or omit
        ) 
    outpath = os.path.join(save_folder, filename)
    
    # ---- 1) CURRENT heatmaps (all mechs, all freqs) ----
    rows_curr = []
                ...
                for i_min in range(n_dG):
                    ...
                    if np.isfinite(val):
                        rows_curr.append({
                            ...
                            "dG_max_eV": float(dG_vals[i_max]),
                            "dG_min_eV": float(dG_vals[i_min]),
                            "Metric": heatmap_metric_curr,   # e.g. avg_curr_metric
                            "Value": float(val),
                        })
    
    df_curr = pd.DataFrame(rows_curr)
    
    # ---- 2) RATE heatmaps (VT=rT metric, VH=rH metric) ----
    rows_rate = []
    for mech_label, freq_dict in heatmaps_rate.items():
        rate_metric = heatmap_metric_VT if mech_label == "VT" else heatmap_metric_VH
        for freq, Z_matrix in freq_dict.items():
            for i_max in range(n_dG):
                for i_min in range(n_dG):
                    val = Z_matrix[i_max, i_min]
                    if np.isfinite(val):
                        rows_rate.append({
                            ...
                            "dG_max_eV": float(dG_vals[i_max]),
                            "dG_min_eV": float(dG_vals[i_min]),
                            "Metric": rate_metric,           # VT->avg_rT_metric, VH->avg_rH_metric
                            "Value": float(val),
                        })
    
    df_rate = pd.DataFrame(rows_rate)
    
    # ---- Optional: combined long-format table ----
    df_long = pd.concat([df_curr, df_rate], ignore_index=True)
    
    # ---- Write to Excel (multiple sheets) ----
    with pd.ExcelWriter(outpath, engine="openpyxl") as writer:
        df_curr.to_excel(writer, sheet_name="CurrentHeatmap", index=False)
        df_rate.to_excel(writer, sheet_name="RateHeatmap", index=False)
        df_long.to_excel(writer, sheet_name="LongFormat", index=False)
