...
freq_array = [k_V_RDS / 1e-9, k_V_RDS / 1e-10, k_V_RDS / 1e-11, k_V_RDS / 1e-12]
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        # time spacing
        t, max_time = make_t_eval(freq, t_switching=tswitching, n_cycles= 3,
        ...
        is_min = np.isclose(GHad_t_J, dGmin, rtol=1e-9, atol=tol)
        is_max = np.isclose(GHad_t_J, dGmax, rtol=1e-9, atol=tol)
        
        # First time index where we actually hit dGmin and dGmax after tswitching
        idx_min = np.where((t >= tswitching) & is_min)[0]
        idx_max = np.where((t >= tswitching) & is_max)[0]

        mask_min = np.zeros_like(t, dtype=bool)
        mask_max = np.zeros_like(t, dtype=bool)
        
        if len(idx_min) > 0:
            mask_min[idx_min] = True
            
        if len(idx_max) > 0:
            mask_max[idx_max] = True
            
        t_min = t[mask_min]
        t_max = t[mask_max]
        
        rT_min = r_T_vals[mask_min]
        rT_max = r_T_vals[mask_max]
        
        rH_min = r_H_vals[mask_min]
        rH_max = r_H_vals[mask_max]

        avg_rT_dGmin = trapezoid(rT_min, t_min) / (t_min[-1] - t_min[0])
        avg_rT_dGmax = trapezoid(rT_max, t_max) / (t_max[-1] - t_max[0])
        
        avg_rH_dGmin = trapezoid(rH_min, t_min) / (t_min[-1] - t_min[0])
        avg_rH_dGmax = trapezoid(rH_max, t_max) / (t_max[-1] - t_max[0])
        
        point_rT_min = rT_min.mean()
        point_rT_max = rT_max.mean()
        ...
        dynamic_overlay_by_freq[freq] = [
            (dGmin_dynamic, float(avg_rT_dGmin)),
            (dGmax_dynamic, float(avg_rT_dGmax)),
        ]
        ...
        dynamic_overlay_by_freq_rH[freq] = [
            (dGmin_dynamic, float(avg_rH_dGmin)),
            (dGmax_dynamic, float(avg_rH_dGmax)),
            ]
