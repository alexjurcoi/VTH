...
def make_output_filename(kV, kT, kH, freq_phys=None, beta=None,
    ...
    """
    freq_str = "-".join([f"{f:.1e}" for f in freq_phys])
...
freq_phys = np.array(np.logspace(-2, 6, 9))
...
if do_dynamic_ghad:
    ...
    for mechanism_choice in mechanisms_to_run:
        ...
        for freq in freq_phys:
            ...
            cycles_to_plot_coverage = cycles_to_run
            t_end_plot = cycles_to_plot_coverage * P
            t_end_binding = cycles_to_plot_binding * P
            t_start = t_end_plot * 0.05
            mask = (t >= t_start) & (t <= t_end_plot)
            mask_binding = (t >= t_start) & (t <= t_end_binding)

            average_rT_dynamic = np.average(r_T_vals[mask])
            average_rH_dynamic = np.average(r_H_vals[mask])
            average_rV_dynamic = np.average(r_V_vals[mask])
...
# =============================================================================
            ...
            if mechanism_choice == 0:
                ...
                plt.axhline(y=average_rT_dynamic, color="red", linestyle="--", linewidth=2,
                            label=f"Average rT = {average_rT_dynamic:.2e}")
                plt.xlabel("Time (s)")
            ...
            if mechanism_choice == 1:
                ...
                plt.axhline(y=average_rH_dynamic, color="red", linestyle="--", linewidth=2,
                            label=f"Average rH = {average_rH_dynamic:.2e}")
                plt.xlabel("Time (s)")
...
if do_static_volcano:
    ...
    if static_results_VT_rT:
        ...
        if static_results_VT_rV:
            GHad_vals_curr, avg_curr_vals_VT = zip(*static_results_VT_rV)
            plt.figure(figsize=(10, 8))
    ...
    if static_results_VH_rH:
        ...
        # ============================
        if static_results_VH_rV:
            GHad_vals_curr, avg_curr_vals_VH = zip(*static_results_VH_rV)
