...
k_V_RDS = 6.590103e-05 / 5000 #units of mols/cm^2
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        avg_rT_dGmin = np.abs(np.average(r_T_vals[mask_min])) # mol/cm²·s
        avg_rT_dGmax = np.abs(np.average(r_T_vals[mask_max]))
        avg_rT = np.abs(np.average(r_T_vals))
        avg_rT_list.append(avg_rT)
...
# =============================================================================
    ...
    for i, res in enumerate(dyn_results[:4]):
        ...
        # Right axis: r_T and θ_H
        ax2=ax.twinx()
        ax2.plot(t_zoom, r_V_zoom, color='fuchsia', lw=1.5, label="r_V")
        ax2.set_ylabel("r_V", color="black")
        ax2.tick_params(axis='y', labelcolor="black")
...
# Increase the number of evaluation points for a finer resolution
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        average_rT = np.average(rT_static)
        average_rV = np.average(r0_vals[:,0])
        average_current=np.average(curr_static)
        GHad_results.append((GHad_eV, average_rV, average_rT))
        print(f"rT at {GHad} = ", average_rT)
# =============================================================================
#     print(f"Curr_model[400] = {curr_static[400]:.3f} mA/cm²")
# =============================================================================

    # Volcano plot
    GHad_vals, avg_rV, avg_rT =zip(*GHad_results)
    plt.figure(figsize=(8, 5))
    plt.plot(GHad_vals, avg_rV, label='Static GHad Scan')
    ...
    plt.xlabel("GHad (eV)")
    plt.ylabel("rT, mol/site*s")
    plt.title(
        f"Volcano Plot: rV vs GHad, $k_V$ ={k_V / cmax:.2e}, $beta$ = {beta[0]}")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
# =============================================================================
    ...
    GHad_vals, avg_rV, avg_rT =zip(*GHad_results)
    plt.figure(figsize=(8, 5))
    plt.plot(GHad_vals, avg_rT, label='Static GHad Scan')
    ...
    plt.ylabel("rT, mol/site*s")
    plt.title(
        f"Volcano Plot: rT vs GHad, $k_T$ ={k_T / cmax:.2e}")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
