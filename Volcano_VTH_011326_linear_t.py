...
if do_dynamic_ghad:
    ...
    #   "avg_curr_2pt"  -> 0.5*(avg|curr| near dGmin + near dGmax)
    #   "avg_curr_abs"  -> abs(average over entire waveform)
    #   "avg_rT_2pt"    -> 0.5*(avg rT near dGmin + near dGmax)
    #   "avg_rH_2pt"    -> 0.5*(avg rH near dGmin + near dGmax)
    heatmap_metric = "avg_curr_2pt"
...
avg_vals_T = np.array(avg_vals_T) / max_static_VT
...
###################### VT Curr Freq Sweep Plot ##############################

mech_T = "VT"
quantity_T = "curr"   # choose: "rT", "rH", or "curr"
...
for f, d in overlay_storage[mech_T]["avg"].items():
    freqs_T.append(f)
    avg_vals_T.append(d["average_curr"])
...
avg_vals_H = np.array(avg_vals_H) / max_static_VH
...
########################### VH CURR FREQ SWEEP PLOT ###################################

mech_H = "VH"
quantity_H = "curr"   # choose: "rT", "rH", or "curr"
...
for f, d in overlay_storage[mech_H]["avg"].items():
    freqs_H.append(f)
    avg_vals_H.append(d["average_curr"])
