...
freq_array = np.array(np.logspace(-12, -3, 13) / k_V_RDS)
...
plt.title(f"Average rT vs Frequency, at V={V_app},\n dGmin={dGmin_dynamic}, dGmax={dGmax_dynamic}, phi = {phi}")
...
if avg_vals_rV_T.min() < avg_vals_rV_H.min():
    ymin_rV = avg_vals_rV_T
else:
    ymin_rV = avg_vals_rV_H
    
if avg_vals_rV_T.max() > avg_vals_rV_H.max():
    ymax_rV = avg_vals_rV_T
else:
    ymax_rV = avg_vals_rV_H
