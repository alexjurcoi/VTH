...
def make_output_filename(kV, kT=None, kH=None, freq_array=None, beta=None,
    ...
    if kT is not None:
        k_str = f"kV_{kV:.2e}_kT={kT:.2e}"
    elif kH is not None:
        k_str = f"kV_{kV:.2e}_kH={kH:.2e}"
    else:
        k_str = f"kV_{kV:.2e}"
    ...
    filename = (
        f"sim_k={k_str}_freq={freq_str}_beta={beta_str}"
        f"_dG={dGmin:.2f}-{dGmax:.2f}eV_V={voltage:.2f}V.xlsx"
    )
...
dGmin_dynamic = 0.00  # in eV
dGmax_dynamic = 0.20  # in eV
...
# =============================================================================
    ...
    plt.scatter(freq_labels, avg_rT_dGmin, color="blue", marker="o",
                label=f"Avg rT at {dGmin_dynamic:.2f} eV")
    plt.scatter(freq_labels, avg_rT_dGmax, color="red", marker="x",
                label=f"Avg rT at {dGmax_dynamic:.2f} eV")  # Using 'x' for better visual distinction
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("TOF mol/cm2*s")
    plt.title("Dynamic GHad(t): Average rT Per Frequency")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    output_filename = make_output_filename(
        ...
        dGmin=dGmin_dynamic,
        dGmax=dGmax_dynamic,
        voltage=potential(0)  # or whatever voltage you are holding
    )
...
# Increase the number of evaluation points for a finer resolution
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        def potential_static(t): return -0.4
