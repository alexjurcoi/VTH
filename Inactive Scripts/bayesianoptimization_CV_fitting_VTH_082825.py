...
GHad_eV_list = np.linspace(-0.5, -0.3, 40)
def simulate_and_score(kV_rel_log10, betas, GHad_eV_val):
    ...
    try:
        ...
        beta = [float(betas[0]), float(betas[1])]  # <<< IMPORTANT
        ...
        y_true = I_exp_masked.to_numpy()
        y_pred = I_model_on_exp
# =============================================================================
#         if len(y_true) > 8:
...
for g in GHad_eV_list:
    ...
    # Build search space and wrapper objective that always passes a 2-element beta list
    if mechanism_choice == 0:   # VT: optimize kV and beta_V
        space = [
            Real(-5.0, 5.0, name="kV_rel_log10"),
            Real(0.05, 0.99, name="beta_V"),
        ]
        def obj(x):  # x = [kV_rel_log10, beta_V]
            return simulate_and_score(x[0], [x[1], beta[1]], g)  # keep current beta_H
    else:                        # VH: optimize kV, beta_V, beta_H
        space = [
            Real(-3.0, 3.0, name="kV_rel_log10"),
            Real(0.20, 0.90, name="beta_V"),
            Real(0.20, 0.90, name="beta_H"),
        ]
        def obj(x):  # x = [kV_rel_log10, beta_V, beta_H]
            return simulate_and_score(x[0], [x[1], x[2]], g)
    ...
    # Unpack and record best solution
    best_rel = res.x[0]
    best_kV  = k_V_RDS * (10.0 ** best_rel)
    if mechanism_choice == 0:
        best_betaV, best_betaH = res.x[1], beta[1]
    else:
        best_betaV, best_betaH = res.x[1], res.x[2]
    best_R2 = 1.0 - res.fun
...
print("\nVolmer–Tafel optimization results:")
print(df_vt)
...
R2_MIN = 0.9  # only show fits with R² >= set value of R2_min

fig, ax = plt.subplots(figsize=(9, 7))

for (g, kV_best, betaV_best, betaH_best, r2_best) in results:
    if not np.isfinite(r2_best) or r2_best < R2_MIN:
        continue  # skip weak fits

    # set best-fit params for this GHad
    k_V = float(kV_best)
    if mechanism_choice == 0:   # VT
        k_T = 1000.0 * k_V
    else:                       # VH
    ...
    beta = [float(betaV_best), float(betaH_best)]  # <<< use both betas
    GHad_eV = float(g)
    GHad = GHad_eV * AvoNum * conversion_factor
