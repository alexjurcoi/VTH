...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        # --- time grid that scales with frequency (no period var) ---
        t_switching = 0.5
        n_cycle = 10
        t_end = t_switching + n_cycle / freq

        # background grid that grows with freq
        base_points = 200
        n_points = max(int(base_points * n_cycle * np.sqrt(freq)), 200)
        t_base = np.linspace(0.0, t_end, num=n_points)

        # exact jump times (GHad flips every 1/freq)
        jump_times = t_switching + np.arange(0, n_cycle + 1) / freq
        jump_times = jump_times[(jump_times >= 0.0) & (jump_times <= t_end)]

        # sample just before/after each jump by a small fraction of the cycle
        eps = 5e-4 / freq  # ~0.1% of a cycle
        edges = np.concatenate((jump_times - eps, jump_times + eps))
        edges = edges[(edges >= 0.0) & (edges <= t_end)]

        # final evaluation times
        t = np.sort(np.unique(np.concatenate((t_base, edges, [t_end]))))
        duration = (0.0, t_end)

        # keep solver steps to at most a quarter of a cycle
        maxstep = 0.05 / freq
        ...
        soln = solve_ivp(sitebal, duration, theta0,
                         t_eval=t, method='BDF',
                         rtol=1e-7, atol=1e-10, max_step=maxstep)
    ...
    fig.suptitle("Dynamic GHad(t): Current vs Time", fontsize=16)
...
if do_static_volcano:
    ...
    t_eval = np.linspace(0, t_end, 10000) # Increased resolution to 10,000 points
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        soln = solve_ivp(sitebal, (0, t_end), theta0, t_eval=t_eval, method='BDF', max_step = 0.1)
