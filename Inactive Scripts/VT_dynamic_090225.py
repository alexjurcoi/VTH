...
def make_t_eval(freq, t_switching=0.5, n_cycles=6,
    ...
    t_coarse = np.arange(0.0, t_end + 1e-12, dt)  # small epsilon to include t_end
...
partialPH2 = 1.0
beta = [0.443716, 0.5]
freq_array = [100000, 10000, 1000]

k_V_RDS = 6.590103e-05 / 2000
...
if do_dynamic_ghad:
    ...
    # Initial conditions
    thetaA_H0_dynamic = 0.5  # Initial coverage of Hads, needs to be high as this is reduction forward
    ...
    for freq in freq_array:
        ...
        # time spacing
        t_switching  = 0.5
        t, max_time = make_t_eval(freq, t_switching=0.5, n_cycles=20,
                                  coarse_pts_per_period=40, halo_frac=0.05, halo_points=15)
        duration = [t[0], t[-1]]
        ...
        soln = solve_ivp(sitebal, duration, theta0_dynamic, t_eval = t, method ='BDF')
...
# =============================================================================
    ...
    plt.scatter(freq_labels, avg_currents, color="blue", marker="o", label=f"Avg |Current| at {dGmin_dynamic:.2f}–{dGmax_dynamic:.2f} eV")
    ...
    fig_half, axes_half = plt.subplots(2, 2, figsize=(14, 8), constrained_layout=True)
    ...
    for i, res in enumerate(dyn_results[:4]):
        ...
        # Start after switching onset (t = 0.5s)
        t_start = 0.5
        t_end = t_start + 3 * period  # zoom into ~3 cycles
    
        # Slice arrays for this time window
        mask = (t_arr >= t_start) & (t_arr <= t_end)
        t_zoom = t_arr[mask]
        curr_zoom = curr_arr[mask]
    
        ax.plot(t_zoom, curr_zoom, color='blue', lw=1.5)
    ...
    fig_half.suptitle("Zoomed View: Second Half of Current Data for Each Frequency", fontsize=14)
    plt.show()
...
if do_static_volcano:
    ...
    # Initial conditions
    thetaA_H0_static = 0.5  # Initial coverage of Hads, needs to be high as this is reduction forward
    ...
    max_time = 20
    t_static = np.arange(0.0, max_time, scanrate)
    endtime_static = t_static[-1]
    duration_static = [0, endtime_static]
    time_index = [t_static]
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        def rates_r0_static(t_static, theta):
            ...
            U_V, U_H = eqpot_static(theta)  # call function to find U for given theta
        ...
        soln = solve_ivp(sitebal_static, duration_static, theta0_static, t_eval=t_static, method='BDF', max_step = maxstep)
        r0_vals = np.array([rates_r0_static(time, theta) for time, theta in zip(t_static, soln.y.T)])
...
# =============================================================================
    ...
    if dynamic_overlay_by_freq:
        ...
        freq_markers = {1000: "x", 10000: "^", 100000: "s"}     # change as you like
        freq_colors  = {1000: "tab:blue", 10000: "tab:orange", 100000: "tab:green"}
    ...
    plt.title(
        f"Volcano Plot: Max Current vs GHad, $k_V$ ={k_V / cmax:.2e}, $beta$ = {beta[0]}, $V$ = {potential_static(t_static)}")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
