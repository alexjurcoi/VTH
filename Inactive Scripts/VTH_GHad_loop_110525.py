...
Avo = 6.02e23  # 1/mol
GHad = -0.1 * Avo * conversion_factor
...
plt.plot(t[5:], volmer_rate[5:], label = f'Average Volmer Rate = {avg_rV:.2e}')
plt.xlabel('Time (s)')
...
if mechanism_choice == 0:
    plt.plot(t[5:], tafel_rate[5:], label = f'Average Tafel Rate = {avg_rT:.2e}')
    plt.xlabel('Time (s)')
...
if mechanism_choice == 1:
    plt.plot(t[5:], rH_vals[5:], label = f'Average Heyrovsky Rate = {avg_rH:.2e}')
    plt.xlabel('Time (s)')
