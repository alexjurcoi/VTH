...
partialPH2 = 1.0
beta = [0.35, 0.5]
freq_array = [1, 10, 100, 1000, 10000, 100000]
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        print(f"\nRunning simulation with period = {freq} Hz...")

        # time spacing
        t_switching = 1  # no dynamic switching before this time
        max_time = np.arange(6)  # seconds
        mt_length = len(max_time)
        interval_factor = 5000
        t = np.linspace(0, max_time[-1], interval_factor * mt_length)
        ...
        soln = solve_ivp(sitebal, duration, theta0, t_eval = t, method ='BDF', max_step = 1/len(t))
        ...
        curr_max_dynamic = np.max(curr_dynamic)
        curr_min_dynamic = np.min(curr_dynamic)

        max_and_min_vals = [curr_max_dynamic, curr_min_dynamic]
        ...
        dyn_results.append({
            ...
            "max and min": max_and_min_vals,
            "thetaH": thetaH_array.copy(),
            "GHad_eV": GHad_t_eV.copy(),
        })

    # Gather values
    freq_labels = [f"{res['freq']:.2e}" for res in dyn_results]  # make nice string labels
    maxvals = [res["max and min"][0] for res in dyn_results]
    minvals = [res["max and min"][1] for res in dyn_results]

    plt.figure(figsize=(10, 6))

    # Plot points
    plt.scatter(freq_labels, maxvals, color="red", marker="o", label="Max current")
    plt.scatter(freq_labels, minvals, color="blue", marker="x", label="Min current")
    ...
    for res in dyn_results:
        ...
        plt.plot(res["t"][2000:2010], res["curr"][2000:2010], label=label)
    plt.xlabel("Time (s)")
    plt.ylabel("Current (mA/cm²)")
    plt.title("Dynamic GHad(t): Current vs Time for Different Periods")
    plt.grid(True)
