...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        soln = solve_ivp(sitebal, duration, theta0_dynamic, t_eval = t, max_step = maxstep, method ='BDF')
        ...
        avg_curr = np.average(curr_dynamic)
        ...
        avg_curr_at_dGmin = np.average(curr_dynamic[mask_min])
        avg_curr_at_dGmax = np.average(curr_dynamic[mask_max])
...
# Increase the number of evaluation points for a finer resolution
    ...
    t_eval = np.linspace(0, t_end, 1000) # Increased resolution to 10,000 points
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        soln = solve_ivp(sitebal_static, (0, t_end), theta0_static, t_eval=t_eval, method='BDF')
        r0_vals = np.array([rates_r0_static(time, theta) for time, theta in zip(t_eval, soln.y.T)])
        curr_static = r0_vals[:, 0] * -F * 1000  # mA/cm²
        ss_current = np.abs(curr_static[-1])
        GHad_results.append((GHad_eV, ss_current))
...
# =============================================================================
    ...
    if dynamic_overlay_by_freq:
        ...
        freq_markers = {freq_array[0]: "D", freq_array[1]: "^", freq_array[2]: "s"}     # change as you like
        freq_colors  = {freq_array[0]: "red", freq_array[1]: "tab:orange", freq_array[2]: "tab:green"}
