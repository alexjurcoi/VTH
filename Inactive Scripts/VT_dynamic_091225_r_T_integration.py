...
if do_dynamic_ghad:
    ...
    theta0_dynamic = [thetaA_Star0_dynamic, thetaA_H0_dynamic, 0.0]
    ...
    for freq in freq_array:
        ...
        def potential(t): return -0.3

        # equil
        def eqpot(theta, GHad):
            theta = np.asarray(theta)
            thetaA_star, thetaA_H = theta[:2]  # unpack surface coverage
        ...
        def rates_r0(t, theta):
            GHad = dGvt(t)
            theta = np.asarray(theta)
            thetaA_star, thetaA_H = theta[:2]  # surface coverages again, acting as concentrations
        ...
        def sitebal(t, theta):
            ...
            if mechanism_choice in [0]:
                thetaStar_rate_VT = ((r_T)) / cmax
                thetaH_rate_VT = (-(r_T)) / cmax
                # [0 = star, 1 = H]
        ...
        H2_produced_vs_t = soln.y[2, :]   # mol/cm² as a function of time
        H2_total = H2_produced_vs_t[-1]   # final value
        print(f"Total H₂ produced for freq={freq} Hz: {H2_total:.3e} mol/cm²")
...
# Increase the number of evaluation points for a finer resolution
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        average_current=np.abs(curr_static[-1])
