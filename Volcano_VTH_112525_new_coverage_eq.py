    ...
    for freq in freq_array:
        ...
        def dGvt(t):
            if t < tswitching:
                ...
                return (dGmax - dGmin) + ((dGmax - dGmin)/2) * np.tanh(k*np.sin(2*np.pi*freq*t))
...
# =============================================================================
    ...
    for res in dyn_results:
        ...
        plt.plot(t_res, thetaH, label=f'Theta_H Coverage ({freq:.2e} Hz)')
        plt.xlabel("Time (s)")
        plt.ylabel(r"$\theta_H$")
...
if do_static_volcano:
    ...
    print(
        f"\nAverage Current (static): {abs_currents[max_index]:.3f} mA/cm² at GHad = {GHad_vals[max_index]:.3f} eV")
