...
if do_static_volcano:
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        def theta_H_eq_static(GHad, V, mechanism_choice):
            ...
            def f(thetaH):
                theta = np.array([1 - thetaH, thetaH])
                rV, rT, rH = rates_r0_static(0, theta)
                if mechanism_choice == 0:
                    return rV - 2*rT
                if mechanism_choice == 1:
                    return rV - rH
    ...
    if mechanism_choice == 0:
        ...
        plt.title(f"$\mathbf{{r_T}}$ vs GHad, V = {V_app} V, maxstep = {maxstep:.2e}")
        #plt.grid(True)
        ax = plt.gca()
        ax.xaxis.set_major_locator(MultipleLocator(0.05))
        #ax.yaxis.set_major_locator(MultipleLocator(0.1e-8))
        plt.tight_layout()
        plt.show()
    ...
    if mechanism_choice == 1:
        ...
        plt.xlabel("GHad (eV)")
        plt.ylabel("Average rH (mol/cm² s)")
        plt.title(f"Avg $\mathbf{{r_H}}$ vs GHad, V = {V_app} V, maxstep = {maxstep:.2e}")
        #plt.grid(True)
        plt.tight_layout()
        ax = plt.gca()
        ax.xaxis.set_major_locator(MultipleLocator(0.05))
