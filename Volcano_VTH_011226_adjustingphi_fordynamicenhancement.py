...
freq_array = np.array(np.logspace(-6, 4, 13) / (k_V_RDS / cmax))
...
if do_dynamic_ghad:
    ...
    for mechanism_choice in mechanisms_to_run:
        ...
        for freq in freq_array:
            ...
            # time spacing
            t, max_time = make_t_eval(freq, n_cycles=100,
                                      coarse_pts_per_period=200,
                                      halo_frac=0.10, halo_points=100)
            duration = [0, max_time]
            ...
            def dGvt(t):
                return (dGmin) + (dGmax - dGmin) * (np.tanh(k * (np.sin(2*np.pi * freq * t) - np.sin(np.pi * (0.5 - duty)))) + 1) / 2
...
quantity_rV_T = "curr"   # choose: "rT", "rH", or "curr"
quantity_rV_H = "curr"
...
for f, pts in overlay_storage[mech_T]["curr"].items():
    val_weak = pts[0][1]    # weak binding value
    val_strong = pts[1][1]  # strong binding value
...
for f, pts in overlay_storage[mech_H]["curr"].items():
    val_weak = pts[0][1]    # weak binding value
    val_strong = pts[1][1]  # strong binding value
    avg_val_H = 0.5 * (val_weak + val_strong)
...
# Sort by frequency (important for log plots)
freqs_rV_T = np.array(freqs_T)
avg_vals_rV_T = np.array(avg_vals_T)
order_rV_T = np.argsort(freqs_rV_T)

freqs_rV_T = freqs_rV_T[order]
avg_vals_rV_T = avg_vals_rV_T[order]

# Sort by frequency (important for log plots)
freqs_rV_H = np.array(freqs_rV_H)
avg_vals_rV_H = np.array(avg_vals_rV_H)
order_rV_H = np.argsort(freqs_rV_H)

freqs_rV_H = freqs_H[order_rV_H]
avg_val_Hs = avg_vals_H[order_rV_H]
