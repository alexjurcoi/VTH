...
freq_array = [10000000000000 * k_V_RDS, 1000000000000 * k_V_RDS , 100000000000 * k_V_RDS, 10000000000 * k_V_RDS]
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        dynamic_overlay_by_freq[freq] = [
            (dGmin_dynamic, average_rT_dGmin),
            (dGmax_dynamic, average_rT_dGmax),
        ]
...
if do_static_volcano:
    ...
    GHad_eV_list = np.linspace(dGmin_eV, dGmax_eV, 24)
    ...
    ax.plot(GHad_vals, abs_rT, label='Static GHad Scan', marker='o', color="blue")
    
    # --- Find max rT and draw horizontal line ---
    max_index_rT = np.argmax(abs_rT)
    max_rT_value = abs_rT[max_index_rT]
    max_GHad_value = GHad_vals[max_index_rT]
    
    # Draw the horizontal line at the peak
    ax.axhline(y=max_rT_value, color='black', linestyle='--', linewidth=2)
    
    # --- Dynamic overlay setup ---
    if dynamic_overlay_by_freq:
        freq_markers = {freq_array[0]: "D", freq_array[1]: "^",
                        freq_array[2]: "s", freq_array[3]: "o"}
        freq_colors = {freq_array[0]: "red", freq_array[1]: "tab:orange",
                       freq_array[2]: "tab:green", freq_array[3]: "tab:pink"}
        ...
        for f in freq_array:
            ...
            xs = [p[0] for p in pts]       # dGmin, dGmax
            ys = [p[1] for p in pts]
            avg_rT_all = np.mean(ys)       # midpoint value
    
            # --- Dynamic endpoints (keep labels like before) ---
            ax.scatter(xs, ys,
            ...
            # --- Add center marker for averaged rT ---
            mid_x = np.mean(xs)
            mid_y = avg_rT_all
            ax.scatter(mid_x, mid_y,
                       marker=freq_markers.get(f, "x"),
                       s=160,
                       facecolors=freq_colors.get(f, None),
                       edgecolors="black",
                       linewidths=1,
                       zorder=4)
    
            # --- Add legend entry for averaged point ---
            legend_handles.append(
                plt.Line2D([0], [0],
                           marker=freq_markers.get(f, "x"),
                           color='w',
                           label=f"{int(round(f))} Hz avg rₜ = {avg_rT_all:.2e}",
                           markerfacecolor=freq_colors.get(f, None),
                           markeredgecolor='black',
                           markersize=10,
                           linewidth=0)
            )
    
    # --- Legend entries for static volcano ---
    static_line = plt.Line2D([], [], color='blue', marker='o', label='Static GHad Scan')
    max_line = plt.Line2D([], [], color='black', linestyle='--',
                          label=f"Max TOF ({max_GHad_value:.2f} eV, {max_rT_value:.2e})")
    
    # --- Combine legends ---
    ax.legend(handles=[static_line, max_line] + legend_handles,
              loc='lower right', fontsize=12, framealpha=0.9)
    
    # --- Final formatting ---
    ax.set_xlabel("GHad (eV)")
    ax.set_ylabel("Average $\mathbf{r_T}$ (mol/cm²·s)")
    ax.set_title(f"Volcano Plot: Avg $\mathbf{{r_T}}$ vs Binding Energy, V = {V_app} V")
