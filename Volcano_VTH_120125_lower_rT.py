...
def make_output_filename(kV, kT=None, kH=None, freq_array=None, beta=None,
                         dGmin=None, dGmax=None, voltage=None, t_switching=None,
...
def make_t_eval(freq, t_switching=0.5, n_cycles=20,
    ...
    dt_eq = max(t_switching / max_eq_pts, dt)
    t_eq = np.arange(0, t_switching, dt_eq)
    #1e-12 to include buffer at end
    t_between = np.arange(t_switching, t_end + 1e-12, dt)
    t_between = t_between[t_between <= t_end]
    t_coarse = np.unique(np.concatenate([t_eq, t_between]))
# =============================================================================
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        # time spacing
        t, max_time = make_t_eval(freq, t_switching=tswitching, n_cycles= 20,
                                  coarse_pts_per_period=200, halo_frac=0.10, max_eq_pts = 500, halo_points=100)
        duration = [t[0], t[-1]]
        ...
        def dGvt(t):
            return (dGmax - dGmin) + ((dGmax - dGmin)/2) * np.tanh(k*np.sin((2*np.pi*freq*t)+np.pi + 1e-9))
        ...
        def theta_H_eq_dynamic(GHad, V, mechanism_choice):
        ...
        mask_min = (t >= tswitching) & (np.abs(GHad_t_J - dGmin) < 0.2 * GHad_range)
        mask_max = (t >= tswitching) & (np.abs(GHad_t_J - dGmax) < 0.2 * GHad_range)
        ...
        # define time window
        t_start = 0
        t_end = cycles_to_plot * P

        mask_GHad = (t >= t_start) & (t <= t_end)
        
        plt.plot(t[mask_GHad], GHad_t_eV[mask_GHad], label='GHad')
        plt.title('GHad vs Time')
        plt.show()
# =============================================================================
...
# =============================================================================
    ...
    for res in dyn_results:
        ...
        #calculating average rH and rT
        average_rT = np.average(rT_vals)
        ...
        # make separate plot for each frequency
            ...
            plt.axhline(y=average_rT, color="red", linestyle="--", linewidth=2, label=f"Average rT = {average_rT:.2e}")
            plt.xlabel("Time (s)")
        ...
        if mechanism_choice == 1:
            ...
            plt.axhline(y=average_rH, color="red", linestyle="--", linewidth=2, label=f"Average rH = {average_rH:.2e}")
            plt.xlabel("Time (s)")
        ...
        #plotting coverage vs time, masked to show fewer cycles
        plt.figure(figsize=(12,10))
        plt.plot(t_res[mask], thetaH[mask], label=f'Theta_H Coverage ({freq:.2e} Hz)')
        plt.xlabel("Time (s)")
        plt.ylabel(r"$\theta_H$")
        plt.title(f'Coverage vs Time, {freq:.2e} Hz')
        plt.grid(True, alpha=0.4)
        plt.legend()
        plt.show()
        
        #plotting rV (NOT CURRENT) vs time
        plt.figure(figsize=(12,10))
        plt.plot(t_res[mask], rV_vals[mask], label=f'rV Coverage ({freq:.2e} Hz)')
        plt.xlabel("Time (s)")
        plt.ylabel(r"$rV$")
        plt.title(f'rV vs Time, {freq:.2e} Hz')
        plt.grid(True, alpha=0.4)
        plt.legend()
        plt.show()
...
if do_static_volcano:
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        ...
        def theta_H_eq_static(GHad, V, mechanism_choice):
            # bracket for theta, solution should be between 1e-9 and 1 - 1e-9
            lo, hi = 1e-9, 1 - 1e-9
        ...
        # Initial coverage of Hads, inside loop so that it starts fresh each time
        thetaA_H0_static = theta_H_eq_static(dGmin, V_app, mechanism_choice)
        thetaA_Star0_static = 1.0 - thetaA_H0_dynamic  # Initial coverage of empty sites
        ...
        print('Average Coverage:', {avg_thetaH})
        ...
        avg_summary.append({
            ...
            "Average rH (mol/cm²·s)": average_rH
        })

    if mechanism_choice == 0:
        
            # Volcano plot (rT)
        GHad_vals, avg_rT = zip(*GHad_results)
        plt.figure(figsize=(10, 8))
        plt.plot(GHad_vals, avg_rT, label='Static GHad Scan', marker='o', color="blue")
        
        
        if dynamic_overlay_by_freq:
            freq_markers = {freq_array[0]: "D", freq_array[1]: "^", freq_array[2]: "*", freq_array[3]: "X"}
            freq_colors = {freq_array[0]: "red", freq_array[1]: "tab:orange", freq_array[2]: "blue", freq_array[3]: "green"}
        ...
        plt.title(f"$\mathbf{{r_T}}$ vs GHad, V = {V_app} V, maxstep = {maxstep:.2e}, t_switch = {tswitching}")
        #plt.grid(True)
        ax = plt.gca()
        ax.xaxis.set_major_locator(MultipleLocator(0.05))
        ax.yaxis.set_major_locator(MultipleLocator(0.1e-9))
        plt.tight_layout()
        plt.show()
    ...
    if mechanism_choice == 1:
        ...
        plt.title(f"Avg $\mathbf{{r_H}}$ vs GHad, V = {V_app} V, maxstep = {maxstep:.2e}, t_switch = {tswitching}")
        #plt.grid(True)
        plt.tight_layout()
        ax = plt.gca()
        ax.xaxis.set_major_locator(MultipleLocator(0.05))
    ...
    GHad_vals, avg_current = zip(*GHad_results1)
    plt.figure(figsize=(10, 8))
    plt.plot(GHad_vals, avg_current, label='Static GHad Scan', marker='o', color="blue")
    ...
    plt.title(f"Volcano Plot: Average Current vs GHad, V = {V_app} V")
    #plt.grid(True)
    plt.tight_layout()
    ax = plt.gca()
    ax.xaxis.set_major_locator(MultipleLocator(0.05))
    ...
    max_index = np.argmax(avg_current)
    print(
        f"\nAverage Current (static): {avg_current[max_index]:.3f} mA/cm² at GHad = {GHad_vals[max_index]:.3f} eV")
...
output_filename = os.path.join(
    save_folder,
    make_output_filename(kV=k_V, kT=k_T, kH=k_H, t_switching = tswitching,
