...
def make_t_eval(freq, t_switching=0.5, n_cycles=4,
                ...
                halo_frac=0.05, max_eq_pts = 2000,
                halo_points=40):
    P = 2.0 / float(freq)
...
V_app = -0.2
tswitching = 100
k = 5 #sharpness of tanh, higher number = more square
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        # time spacing
        t, max_time = make_t_eval(freq, t_switching=tswitching, n_cycles= 6,
                                  coarse_pts_per_period=200, halo_frac=0.10, max_eq_pts = 2000, halo_points=100)
        duration = [t[0], t[-1]]
        ...
        def dGvt(t):
            if t < tswitching:
                ...
                #tanh square wave, k = sharpness of square
                return  dGmin + ((dGmax - dGmin)/2) * np.tanh(k*np.sin(2*np.pi*freq*t)) 
...
# =============================================================================
    ...
    for res in dyn_results:
        ...
        plt.plot(t[mask], GHad_t_eV_local[mask], label = f'dG at {freq}')
        plt.title(f'dG vs Time, {freq:.2e}')
        plt.legend()
        plt.xlabel('Time')
        plt.ylabel('dG (eV)')
        plt.show()
...
if do_static_volcano:
    ...
    if mechanism_choice == 0:
        ...
        plt.title(f"Avg $\mathbf{{r_T}}$ vs GHad, V = {V_app} V, maxstep = {maxstep:.2e}")
        #plt.grid(True)
        ax = plt.gca()
