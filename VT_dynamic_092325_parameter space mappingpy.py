...
freq_array = [100000000000 * k_V_RDS, 10000000000 * k_V_RDS]
...
if do_dynamic_ghad:
    ...
    for freq in freq_array:
        ...
        r_V_vals = r0_vals[:, 0]
        r_T_vals = r0_vals[:, 1]
        avg_rT = np.average(r_T_vals)
        print(f"Average rT for {freq}:", avg_rT)
...
# =============================================================================
    ...
    # Figure out how many frequencies you actually have
    n_freq = len(freq_array)
    
    # Decide how many subplots you want (up to 6 for a 3x2 grid)
    n_subplots = min(n_freq, 6)

    # === Additional plot: Half-data view for better detail ===
    fig_half, axes_half=plt.subplots(
        3, 2, figsize=(18, 12), constrained_layout=True)
    axes_half=axes_half.flatten()

    for i, res in enumerate(dyn_results[:n_subplots]):
        ax=axes_half[i]
        t_arr=res["t"]
    ...
    for j in range(n_subplots, len(axes_half)):
        axes_half[j].set_visible(False)
    ...
    for j in range(n_subplots, len(axes_half2)):
        axes_half2[j].set_visible(False)
        
    # === Additional plot: Half-data view for better detail ===
    fig_half3, axes_half3=plt.subplots(
        3, 2, figsize=(18, 12), constrained_layout=True)
    axes_half3=axes_half3.flatten()
    ...
    for i, res in enumerate(dyn_results[:n_subplots]):
        ...
        # Right axis: r_T and θ_H
        ax2=ax.twinx()
        ax2.plot(t_zoom, rV_zoom, color='orange', lw=1.5, label="r_V")
        ax2.set_ylabel("r_V (mol/cm²·s)", color="black")
        ax2.tick_params(axis='y', labelcolor="black")
    ...
    for j in range(n_subplots, len(axes_half3)):
        axes_half3[j].set_visible(False)
    plt.show()
    ...
    with pd.ExcelWriter(output_filename, engine="openpyxl") as writer:
        for res in dyn_results:
            freq_label = f"{res['freq']:.2e}Hz"  # sheet name (Excel safe)
            
            # Build dataframe for this frequency
            df = pd.DataFrame({
                "Time (s)": res["t"],
                "GHad (eV)": res["GHad_eV"],
            ...
            # [:30] keeps sheet name within Excel’s 31-char limit
...
# Increase the number of evaluation points for a finer resolution
    ...
    for GHad, GHad_eV in zip(GHad_J_list, GHad_eV_list):
        def potential_static(t): return -0.2
        ...
        rT_static_avg = np.average(r0_vals[:, 1])
        
        print(f"Average rT for {GHad}:", rT_static_avg)
...
# =============================================================================
    ...
    plt.plot(GHad_vals, abs_currents, label='Static GHad Scan', marker = 'o')

    if dynamic_overlay_by_freq:
        # choose a unique marker & color per frequency
        freq_markers={freq_array[0]: "D", freq_array[1]
            : "^", freq_array[2]: "s", freq_array[3]: "^"}     # change as you like
        freq_colors={freq_array[0]: "red", freq_array[1]
            : "tab:orange", freq_array[2]: "tab:green", freq_array[3]: "tab:pink"}
    ...
    plt.title(
        f"Volcano Plot: Max Current vs GHad, $k_V$ ={k_V:.2e}, $beta$ = {beta[0]}, V = {potential(t)}")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
